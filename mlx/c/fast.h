...
// `sinks` is optional — pass an empty mlx_array (mlx_array_new()) to skip the
// sinks branch in pass2. When provided, shape [nq_heads] float32; the kernel
// folds sinks[h] into the global softmax denominator. `nq_heads` and `L` are
// always required to compute the head index from the flat q_idx.
int mlx_fast_turbo_flash_pass2(mlx_array* res, const mlx_array o_partials, const mlx_array m_partials, const mlx_array l_partials, int num_blocks, int dim, int nq_heads, int L, const mlx_array sinks, const mlx_stream s);
int mlx_fast_turbo_flash_pass2_fused(mlx_array* res, const mlx_array o_partials, const mlx_array m_partials, const mlx_array l_partials, const mlx_array val_rotation, int num_blocks, int dim, int nq_heads, int L, const mlx_array sinks, const mlx_stream s);
